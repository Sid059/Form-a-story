...
/*words.get('animal-1') is getting the value of a parameter named animal-1 from the URL.
window.location.search gives the part of the URL after the ?, which contains query parameters (like ?animal-1=dog&animal-2=cat).
URLSearchParams is a built-in object that helps read these parameters easily.
words is an object that lets you use .get('parameter-name') to get the value for that parameter.
This is used here to get user input from the URL, so the script can use those values in the story.*/
